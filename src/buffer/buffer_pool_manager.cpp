...
bool BufferPoolManager::FlushPageImpl(page_id_t page_id) {
  ...
  if (debug_msg) {
    LOG_INFO("flush page_id: %d, pin count: %d", page_id, pages_[page_table_[page_id]].GetPinCount());
  }
...
frame_id_t BufferPoolManager::Find_replacementL() {
  ...
  if (!free_list_.empty()) {
    ...
    if (debug_msg) {
      LOG_INFO("Victim - page_id: %d", pages_[fid].GetPageId());
    }
  }
  return fid;
}
  ...
  for (auto &it : page_table_) {
    ...
    if (pages_[frame_id].GetPageId() != pid) {
      if (pages_[frame_id].GetPinCount() == 0) {
        ...
        LOG_ERROR("%d - %d - %d - %d", pid, pages_[frame_id].GetPageId(), frame_id, pages_[frame_id].GetPinCount());
        throw Exception(ExceptionType::INVALID, "gc not ok");
      }
    }
  }
  for (auto &i : vec) {
